...
make.fastq.files <- function(read_length, fasta.path, library_name = "", overlap = F, step_size = read_length){
  ...
  # data <- data[matrix(unlist(stringr::str_split(data$Header, "/")),ncol = 5, byrow = TRUE)[,1] %in% c("AAV.100947", "AAV.100001", "AAV.100151","AAV.100212", "AAV.100003", "AAV.100354", "AAV.100384", "AAV.100015", "AAV.102608","AAV.100033"),]           
...
neighbor.joining <- function(parents_df, path_fasta, path_mapped, path_unmapped, read_length, library_name = "", overlap = F, step_size = read_length){
  ...
  for (j in seq_len(nrow(file_pairs))) {
    ...
    # m_before_kmer[[j, 1]] <- substring(data_mapped[[1,1]], 1, 10) #ex: AAV.100001.1003 --> AAV.100001
    # m_before_kmer[[j, 2]] <- paste(v, collapse = " ")
    
    ##print(j)
    #print(m_before_kmer[[j, 1]])
    #cat("The initial vector before kmer: ", "\n", m_before_kmer[[j, 2]], "\n")
    ...
    # for (i in 1:length(v)){
    #   if (length(v[[i]]) > 1){
    #     read0 = append(read0, paste(substring(data_mapped[[1,1]], 1, 10), "_", as.character(1000+i), sep = "") )
    #     options0[[length(read0)]] = v[[i]]
    #   }
    # }  
    ...
    m[[j, 1]] <- file_pairs[[j, 3]]
      # substring(data_mapped[[1,1]], 
      #                      1, 
      #                      min(unlist(gregexpr('_', data_mapped[1,1]))[1]-1, 
      #                          unlist(gregexpr('/', data_mapped[1,1]))[1]-1))
      #                      #nchar(data_mapped[1,1]) - 6) #ex: AAV.100001.1003 --> AAV.100001
    
    # if (substring(data_mapped[[1,1]], 1, 10) %in% s_rc$V1){
    #   v <- rev(v)
    # }
  ...
  #summary frequency table (not correct/ just rep info)
  # serotypes_freq <- as.data.frame(table(m_new))
  # colnames(serotypes_freq) <- c("Num", "Freq")
  # serotypes_freq['Freq(%)'] <- round(serotypes_freq$Freq*100/sum(serotypes_freq$Freq), 2)
  # legend_name <- c("no alignment", "AAV1", "AAV2", "AAV3", "AAV4", "AAV5", "AAV6", "AAV7", "AAV8", "AAV9", "AAV10", "AAV11", "AAV12", "AAV13", "AAVrh8", "AAVrh10", "AAVrh32", "multiple alignment", "gap")
  # serotypes_freq[' '] <- legend_name[as.numeric(levels(serotypes_freq$Num))+1]
  # print(serotypes_freq[,c(' ', 'Freq(%)')], row.names = F)
